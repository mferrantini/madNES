...
class CPU {
    ...
    getInstruction(opCode) {
        ...
        console.log(`Fetching ${hexOpCode} - PC ${this.REG_PC.value.toString(16)}`);
        return this.INSTRUCTIONS[opCode];
    }

    step() {
        console.log(`${this.currentCycle} CPU STEP`)
        ...
        this.nextInstructionContext = this.currentInstruction[this.currentInstructionStep](this.nextInstructionContext);
        this.currentInstructionStep++;

        if (this.currentInstructionStep === this.currentInstruction.length ||
            this.nextInstructionContext === INSTRUCTIONS.STOP_CURRENT_INSTRUCTION
        ) {
            // 6502 always increments the Program Counter during last cycle.
            // TODO: Check if doing that AFTER the last cycle execution is fine.
            this.REG_PC.increment();

            // Instruction ended
            this.currentInstruction = null;
            this.nextInstructionContext = null;
        }
    ...
    processAddressingMode(addressingMode) {
        switch(addressingMode) {
            case ADDRESSING.IMPLIED:
                return [
                    () => {}, // PC already points to the current instruction
                    () => {},
                ]
            ...
            case ADDRESSING.ABSOLUTE:
                return [
                    ...
                    () => {
                        ...
                        const context = {'address': new Register16Bit()};
            ...
            case ADDRESSING.RELATIVE:
                return [
                    ...
                    () => {
                        ...
                        return {
                            'offset': this.BUS.cpuReadMemory(this.REG_PC.value),
                        }
                    }
                ]
            default:
    ...
    // _processAddressingMode(addressingMode) {
    //     switch (addressingMode) {
    //         case ADDRESSING.IMPLIED:
    ...
    //                     // lower byte.
    //                     instructionContext.address.value = instructionContext.address.value + this.REG_X.value;
    ...
    // TODO: Evaluate a better way to do that

    overlapInstructionSteps(addressingSteps, instructionSteps) {
        let concat = [...addressingSteps];

        concat[concat.length - 1] = context => {
            context = addressingSteps[addressingSteps.length - 1](context);
            instructionSteps[0](context);
        }

        return [...concat, ...instructionSteps.slice(1)];
    }
    ...
    CLEAR_INSTRUCTION(addressingMode, instructionName) {
        ...
        return this.overlapInstructionSteps(
            addressingSteps,
            instructionSteps,
        )
    }
    ...
    LOAD_INSTRUCTION(addressingMode, instructionName) {
        ...
        let instructionSteps = [
            context => {
                ...
                let fetchedData = this.BUS.cpuReadMemory(context.address);

                switch(instructionName) {
                    case INSTRUCTIONS.LDA:
                        this.REG_A.value = fetchedData;
                        isResultZero = this.REG_A.isZero();
                        isResultNegative = this.REG_A.isNegative();
                        break;
                    case INSTRUCTIONS.LDX:
                        this.REG_X.value = fetchedData;
                        isResultZero = this.REG_X.isZero();
                        isResultNegative = this.REG_X.isNegative();
                        break;
                    case INSTRUCTIONS.LDY:
                        this.REG_Y.value = fetchedData;
                        isResultZero = this.REG_Y.isZero();
                        isResultNegative = this.REG_Y.isNegative();
                        break;
                    default:
    ...
    TXS(addressingMode) {
        ...
        let instructionSteps = [
            () => {
                this.REG_SP.lowerByte = this.REG_X.value;
            }
        ];
        return this.overlapInstructionSteps(
            addressingSteps,
    ...
    BRANCH_INSTRUCTION(addressingMode, instructionName) {
        ...
        let instructionSteps = [
            context => {
                ...
                const newPC = new Register16Bit(
                    this.REG_PC.value +
                    Byte.getSignedNumber(context.offset),
                );

                return {
                    ...context,
                    'address': newPC.value,
                    'pageCrossed': newPC.higherByte !== this.REG_PC.value,
                }
            },
            context => {
                this.REG_PC.value = context.address;
                this.REG_PC.increment();

                if (!context.pageCrossed) {
                    // Stop the execution and prevent the next step from being
                    // executed if the branch operation doesn't cross the page.
                    return INSTRUCTIONS.STOP_CURRENT_INSTRUCTION;
                }
            },
            context => {
                // TODO: check if this must be done or not.
                // this.REG_PC.increment();
            }
        ];
        return this.overlapInstructionSteps(
            ...
            instructionContext => {
                ...
                switch(instructionName) {
                    case INSTRUCTIONS.CPX:
                        comparisonResult.value = this.REG_X.value - instructionContext.data;
                        break;
                    default:
    ...
    JSR(addressingMode) {
        ...
        let instructionSteps = [
            ...
            instructionContext => {
                this.REG_PC.value = instructionContext.address.value;
