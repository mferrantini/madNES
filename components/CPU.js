...
class CPU {
    ...
    logState() {
        const pc = this.REG_PC.value;
    
        // Leggi i byte dell'istruzione corrente (fino a 3)
        const b0 = this.BUS.cpuReadMemory(pc);
        const b1 = this.BUS.cpuReadMemory(pc + 1);
        const b2 = this.BUS.cpuReadMemory(pc + 2);
    
        const toHex = (val, pad) => {
            return val.toString(16).toUpperCase().padStart(pad, '0')
        };
    
        // Disassemblaggio inline
        const opcodeMap = {
            ...
            // Illegali
            0x07: { name: 'SLO', mode: 'ZPG' },
            0x17: { name: 'SLO', mode: 'ZPX' },
            0x0F: { name: 'SLO', mode: 'ABS' },
            0x1F: { name: 'SLO', mode: 'ABX' },
            0x1B: { name: 'SLO', mode: 'ABY' },
            0x03: { name: 'SLO', mode: 'INX' },
            0x13: { name: 'SLO', mode: 'INY' },
            0x27: { name: 'RLA', mode: 'ZPG' },
            0x37: { name: 'RLA', mode: 'ZPX' },
            0x2F: { name: 'RLA', mode: 'ABS' },
            0x3F: { name: 'RLA', mode: 'ABX' },
            0x3B: { name: 'RLA', mode: 'ABY' },
            0x23: { name: 'RLA', mode: 'INX' },
            0x33: { name: 'RLA', mode: 'INY' },
            0x47: { name: 'SRE', mode: 'ZPG' },
            0x57: { name: 'SRE', mode: 'ZPX' },
            0x4F: { name: 'SRE', mode: 'ABS' },
            0x5F: { name: 'SRE', mode: 'ABX' },
            0x5B: { name: 'SRE', mode: 'ABY' },
            0x43: { name: 'SRE', mode: 'INX' },
            0x53: { name: 'SRE', mode: 'INY' },
            0x67: { name: 'RRA', mode: 'ZPG' },
            0x77: { name: 'RRA', mode: 'ZPX' },
            0x6F: { name: 'RRA', mode: 'ABS' },
            0x7F: { name: 'RRA', mode: 'ABX' },
            0x7B: { name: 'RRA', mode: 'ABY' },
            0x63: { name: 'RRA', mode: 'INX' },
            0x73: { name: 'RRA', mode: 'INY' },
            0xC7: { name: 'DCP', mode: 'ZPG' },
            0xD7: { name: 'DCP', mode: 'ZPX' },
            0xCF: { name: 'DCP', mode: 'ABS' },
            0xDF: { name: 'DCP', mode: 'ABX' },
            0xDB: { name: 'DCP', mode: 'ABY' },
            0xC3: { name: 'DCP', mode: 'INX' },
            0xD3: { name: 'DCP', mode: 'INY' },
            0xE7: { name: 'ISB', mode: 'ZPG' },
            0xF7: { name: 'ISB', mode: 'ZPX' },
            0xEF: { name: 'ISB', mode: 'ABS' },
            0xFF: { name: 'ISB', mode: 'ABX' },
            0xFB: { name: 'ISB', mode: 'ABY' },
            0xE3: { name: 'ISB', mode: 'INX' },
            0xF3: { name: 'ISB', mode: 'INY' },
            0xA3: { name: 'LAX', mode: 'INX' },
            0xB3: { name: 'LAX', mode: 'INY' },
            0xA7: { name: 'LAX', mode: 'ZPG' },
            0xB7: { name: 'LAX', mode: 'ZPY' },
            0xAF: { name: 'LAX', mode: 'ABS' },
            0xBF: { name: 'LAX', mode: 'ABY' },
            0x87: { name: 'SAX', mode: 'ZPG' },
            0x97: { name: 'SAX', mode: 'ZPY' },
            0x8F: { name: 'SAX', mode: 'ABS' },
            0x83: { name: 'SAX', mode: 'INX' },
            0xEB: { name: 'SBC', mode: 'IMM' }, // alias illegale
            // NOP illegali
            0x1A: { name: 'NOP', mode: 'IMP' },
            0x3A: { name: 'NOP', mode: 'IMP' },
            0x5A: { name: 'NOP', mode: 'IMP' },
            0x7A: { name: 'NOP', mode: 'IMP' },
            0xDA: { name: 'NOP', mode: 'IMP' },
            0xFA: { name: 'NOP', mode: 'IMP' },
            0x80: { name: 'NOP', mode: 'IMM' },
            0x82: { name: 'NOP', mode: 'IMM' },
            0x89: { name: 'NOP', mode: 'IMM' },
            0xC2: { name: 'NOP', mode: 'IMM' },
            0xE2: { name: 'NOP', mode: 'IMM' },
            0x04: { name: 'NOP', mode: 'ZPG' },
            0x44: { name: 'NOP', mode: 'ZPG' },
            0x64: { name: 'NOP', mode: 'ZPG' },
            0x14: { name: 'NOP', mode: 'ZPX' },
            0x34: { name: 'NOP', mode: 'ZPX' },
            0x54: { name: 'NOP', mode: 'ZPX' },
            0x74: { name: 'NOP', mode: 'ZPX' },
            0xD4: { name: 'NOP', mode: 'ZPX' },
            0xF4: { name: 'NOP', mode: 'ZPX' },
            0x0C: { name: 'NOP', mode: 'ABS' },
            0x1C: { name: 'NOP', mode: 'ABX' },
            0x3C: { name: 'NOP', mode: 'ABX' },
            0x5C: { name: 'NOP', mode: 'ABX' },
            0x7C: { name: 'NOP', mode: 'ABX' },
            0xDC: { name: 'NOP', mode: 'ABX' },
            0xFC: { name: 'NOP', mode: 'ABX' },
        };
    
        
        const isIllegal = !opcodeMap[b0];
        const instr = opcodeMap[b0] ?? { name: '*NOP', mode: 'IMP' };
        const instrName = isIllegal ? `*${instr.name}` : instr.name;

        let bytesStr, mnemonic;
        switch (instr.mode) {
            case 'IMM':
                ...
                break;
            case 'ZPG':
                let val = this.BUS.cpuReadMemory(b1);
                bytesStr = `${toHex(b0,2)} ${toHex(b1,2)}   `;
            ...
            case 'REL':
                bytesStr = `${toHex(b0,2)} ${toHex(b1,2)}   `;
                // Calcola l'indirizzo assoluto di destinazione per il log
                const signedOffset = b1 > 0x7F ? b1 - 0x100 : b1;
                const target = (pc + 2 + signedOffset) & 0xFFFF;
        ...
        const a  = toHex(this.REG_A.value, 2);
        const x  = toHex(this.REG_X.value, 2);
        const y  = toHex(this.REG_Y.value, 2);
        const p  = toHex(this.REG_P.value, 2);
        const sp = toHex(this.REG_SP.value, 2);
    
        const ppuY = this.BUS.PPU.currentY.toString().padStart(3, ' ');
        const ppuX   = this.BUS.PPU.currentX.toString().padStart(3, ' ');
        const cpuCycles   = this.totalCycles;
    
        const mnemonicPadded = mnemonic.padEnd(31, ' ');
    
        return `${toHex(pc,4)}  ${bytesStr}  ${mnemonicPadded} A:${a} X:${x} Y:${y} P:${p} SP:${sp} PPU:${ppuY},${ppuX} CYC:${cpuCycles}`;
    }
}
